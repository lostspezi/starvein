...
| `Badge`                                 | Status-Chips; `tone="success                                                                                                                                                                                    | warning"`, Rarity-Farbe via `className`. |
...
- Mobile-first: Basisklassen = Mobile, dann `sm:` / `md:` / `lg:`. Kein horizontales
  Scrollen bei 375/768 px (permanenter E2E-Test).
- Breite Inhalte scrollen in ihrem eigenen Container (`DataTable` bringt
  `overflow-x-auto` mit).
- Sticky-Header ist ~60 px (Desktop) / ~135 px (Mobile, 3 Zeilen) hoch — Anker-Ziele
  brauchen `scroll-mt-40 sm:scroll-mt-24`.
