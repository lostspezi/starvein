...
describe("CompanionGuide", () => {
  it("shows the beta version badge", () => {
    renderWithIntl(<CompanionGuide />);
    expect(screen.getByText("Beta 0.1.0")).toBeVisible();
  });

  it("links the download button to the GitHub releases", () => {
    renderWithIntl(<CompanionGuide />);
    const link = screen.getByRole("link", {
      name: /Download for Windows/i,
    });
    expect(link).toHaveAttribute("href", COMPANION_RELEASES_URL);
  });
  ...
  it("renders in German", () => {
    renderWithIntl(<CompanionGuide />, { locale: "de" });
